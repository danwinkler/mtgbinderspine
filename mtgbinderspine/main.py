...
def render_spine(set_three_letter_code: str, dpi: int=600):
    ...
    svg2png(file_obj=svg_bytesio,
        write_to=raw_image,
        background_color='transparent', scale=(dpi-set_icon_margin*2)/rlg.height)
    ...
    im.alpha_composite(pil_icon, ((icon_area_width - pil_icon.width) // 2, set_icon_margin))
    ...
    draw.text((x_offset + (width - x_offset - text_size[0]) / 2, (height - text_size[1]) / 2), set_name, fill=(0, 0, 0, 255), font=font)
...
def render_spine_command(set_code: List[str]):
    ...
    for i in range(0, len(ims)+1):
        y = i * (ims[0].height+1)
        # Draw the line 
        draw.line((0, y, ims[0].width, y), fill=(200, 200, 200, 255), width=1)
